...
function MealCard({ meal, weekday, date, reference }) {
  ...
  return <article className={`meal-card ${meal ? '' : 'empty'} ${reference ? 'reference' : ''}`}>
    ...
    {meal ? <><h3>중식</h3><ul>{dishes.map((dish) => <li key={dish}>{dish.replace(/\s*\([^)]*\)/g, '')}</li>)}</ul><small>{meal.CAL_INFO || '열량 정보 없음'}</small></> : <div className="no-meal">등록된 급식이<br />없어요</div>}
  </article>;
}

export default function App() {
  const [office, setOffice] = useState('B10'); const [query, setQuery] = useState(''); const [schools, setSchools] = useState([]); const [school, setSchool] = useState(null); const [date, setDate] = useState(toInputDate(new Date())); const [meals, setMeals] = useState([]); const [status, setStatus] = useState(null); const [loading, setLoading] = useState(false);
  ...
  return <main className="shell"><header><div className="brand"><span><Utensils size={17} /></span>오늘 뭐 먹지?</div><small>학교 급식 한눈에 보기</small></header>
    <section className="hero"><div><p>NEIS SCHOOL MEAL</p><h1>이번 주,<br /><em>맛있는</em> 계획.</h1><label>우리 학교의 점심 메뉴를<br />간단하게 확인해 보세요.</label></div><div className="doodle">오늘의<br />점심을<br /><b>찾아볼까요?</b> ↘</div></section>
