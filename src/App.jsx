...
export default function App() {
  ...
  return <main className="shell"><header><div className="brand"><span><Utensils size={17} /></span>오늘 뭐 먹지?</div><small>학교 급식 한눈에 보기</small></header>
    <section className="hero"><div><p>NEIS SCHOOL MEAL</p><h1>이번 주<br /><span className="hero-plan"><em>맛있는</em> 계획</span></h1><label>우리 학교의 점심 메뉴를<br />간단하게 확인해 보세요.</label></div><img className="hero-photo" src="https://images.unsplash.com/photo-1512621776951-a57141f2eefd?auto=format&fit=crop&w=720&q=85" alt="신선한 음식이 담긴 샐러드" /></section>
    ...
    <section className="panel meals"><div className="heading"><div><i>02 / CHECK THIS WEEK</i><h2>급식 조회</h2></div><small>월요일 - 금요일 · 중식</small></div><form className="date-form" onSubmit={findMeals}><label><CalendarDays size={18} /> 조회 기준일 <input type="date" value={date} onChange={(e) => setDate(e.target.value)} /></label><button disabled={loading}>이번 주 메뉴 보기 <b>→</b></button></form>{status && <div className={`status ${status.type}`}>{status.text}</div>}<div className="grid">{dates.map((item,index) => <MealCard key={item} meal={byDate[item]} weekday={WEEKDAYS[index]} date={item} reference={item === date.replaceAll('-','')} />)}<blockquote className="quote-card"><span className="quote-line">인생에서 성공하는</span><span className="quote-line">비결 중 하나는</span><span className="quote-line">좋아하는 음식을 먹고</span><span className="quote-line">힘내 싸우는 것이다</span><small>Mark Twain</small></blockquote></div><p className="note">NEIS Open API의 학교·급식 정보를 바탕으로 제공됩니다.</p></section><footer>ⓘ 급식 정보는 학교 사정에 따라 변경될 수 있습니다.</footer>
  </main>;
}
